...
def test_init_skips_existing(empty_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    (empty_repo / "AGENTS.md").write_text("preserved\n")
    scaffold_manifests.cmd_init(empty_repo, "S0", [], dry=False)
    assert (empty_repo / "AGENTS.md").read_text() == "preserved\n"
    out = capsys.readouterr().out
...
def test_gitignore_appended_with_needed_lines(empty_repo: Path) -> None:
    scaffold_manifests.cmd_init(empty_repo, "S0", [], dry=False)
...
def test_gitignore_dry_run(empty_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    msg = scaffold_manifests._update_gitignore(empty_repo, dry=True)
...
def test_refresh_plan_only_message(empty_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    # Create stale GROUNDING.md (existing file with missing anchors) so refresh has work
...
def test_refresh_apply_writes_backup_and_template(empty_repo: Path) -> None:
    ...
    # _refresh_stale uses with_suffix(suffix + ".bak") -> .md -> .md.bak
...
def test_refresh_apply_overwrites_existing_backup(empty_repo: Path) -> None:
    ...
    # second refresh should overwrite the previous .bak
    rc = scaffold_manifests.cmd_refresh(empty_repo, "S0", [], apply=True, dry=False)
    assert rc == 0


def test_refresh_renders_missing_when_apply(empty_repo: Path) -> None:
    # AGENTS.md missing, GROUNDING.md exists but stale
